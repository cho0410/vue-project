...
export default {
  ...
  setup() {
    const youtubes = ref([]);
    const search = ref("programing");
    const SearchYoutubes = () => {
      fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${search.value}&key=AIzaSyDp8exroG-zW6ZD6mUJe0QRmYhG9SQmTVM&maxResults=28&type=video`
      )
